...
export function getStatusLabel(status: ResourceStatus | undefined, error: string | null): string {
    if (error) {
        return "Fel";
    }
...
export function getGlucoseStatusLabel(status: "high" | "low" | "normal"): string {
    switch (status) {
        case "high":
            return "Hog";
        case "low":
